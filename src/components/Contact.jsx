...
const Contact = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const permission = form.querySelector('#permission')?.checked ? 'Yes' : 'No';
    ...
    // Send email using FormSubmit (Free, no keys required)
    try {
      const response = await fetch(`https://formsubmit.co/ajax/${personalInfo.emails.primary}`, {
        ...
        body: JSON.stringify({
          ...
          Email: email,
          Message: message,
          "Contact Permission Given": permission,
          _subject: `New Portfolio Message from ${firstName}`
        })
      });
