...
const CertificateCard = ({ cert, aosDelay }) => (
  <div 
    ...
    className="relative overflow-hidden group bg-black/20 backdrop-blur-sm rounded-2xl p-5 border border-white/10 hover:border-white/25 hover:scale-105 hover:shadow-[0_15px_40px_rgba(0,0,0,0.3)] transition-all duration-500 cursor-pointer"
  >
    <div className="absolute inset-0 -translate-x-[150%] skew-x-[-25deg] bg-gradient-to-r from-transparent via-white/10 to-transparent group-hover:animate-[swipe_1s_ease-in-out_forwards] z-0 pointer-events-none transition-all duration-700 group-hover:translate-x-[150%]"></div>
    ...
    <section className="bg-[#ff2a2a] pt-20 pb-28 px-6 md:px-12 w-full relative overflow-hidden font-sans">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setIsModalOpen(false)}>
          <div 
            className="bg-[#111] border border-white/10 rounded-3xl w-full max-w-4xl max-h-[90vh] overflow-y-auto p-6 md:p-10 relative shadow-[0_20px_60px_rgba(255,42,42,0.15)]"
          ...
          >
            ...
            <button 
              ...
              className="absolute top-6 right-6 p-2 bg-white/5 hover:bg-white/10 hover:text-red-400 text-white rounded-full transition-colors border border-white/10"
            >
              <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12"></path></svg>
            </button>

            <h3 className="text-3xl font-black text-white mb-2 tracking-tight">All Certificates</h3>
            <p className="text-white/50 mb-8 font-medium">Click on any certificate to view it in detail.</p>

            {/* Certificates Grid */}
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-6">
              {certificates.all.map((cert) => (
                <a
                  key={cert.name}
                  href={cert.file}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center p-5 bg-white/5 border border-white/10 rounded-2xl hover:bg-white/10 hover:border-red-500/30 hover:shadow-[0_0_30px_rgba(255,42,42,0.1)] transition-all duration-300 group"
                >
                  <div className="text-4xl mr-5 group-hover:scale-110 transition-transform duration-300">{cert.icon}</div>
                  <div>
                    <h4 className="text-white font-bold text-base leading-tight mb-1 group-hover:text-white transition-colors">
                      {cert.name}
                    </h4>
                    <p className="text-red-400/80 text-[10px] font-black uppercase tracking-widest mt-1">
                      View PDF Document
                    </p>
                  </div>
                  <svg className="w-5 h-5 text-white/30 ml-auto group-hover:text-red-400 transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
